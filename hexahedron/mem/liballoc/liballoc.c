/**
 * @file hexahedron/mem/liballoc/liballoc.c
 * @brief liballoc allocator
 * 
 * 
 * @copyright
...
/* Debug macro */
#define LIBALLOC_INFO
#if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO

#include <kernel/debug.h>

#ifdef LIBALLOC_DEBUG
#define LOG(status, ...) dprintf_module(status, "ALLOC:LIBALLOC", __VA_ARGS__)
#else
#define LOG(status, ...) if (status != DEBUG && status != NOHEADER) dprintf_module(status, "ALLOC:LIBALLOC", __VA_ARGS__)
#endif
...
void liballoc_dump()
{
	...
	LOG(INFO, "System memory allocated: %i bytes\n", l_allocated );
	LOG(INFO, "Memory in used (malloc'ed): %i bytes\n", l_inuse );
	LOG(INFO, "Warning count: %i\n", l_warningCount );
	LOG(INFO, "Error count: %i\n", l_errorCount );
	LOG(INFO, "Possible overruns: %i\n", l_possibleOverruns );

#ifdef LIBALLOC_DEBUG
		while ( maj != NULL )
		{
			LOG(INFO, "%x: total = %i, used = %i\n",
						maj, 
						maj->size,
						maj->usage );
...
static struct liballoc_major *allocate_new_page( unsigned int size )
{
	...
	struct liballoc_major *maj;
		...
		if ( maj == NULL ) 
		{
			l_warningCount += 1;
			#if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
			LOG(WARN, "WARNING: liballoc_alloc( %i ) return NULL\n", st );
			FLUSH();
			#endif
...
void *PREFIX(malloc)(size_t req_size)
{
	...
	if ( l_memRoot == NULL )
	{
		#if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
		#ifdef LIBALLOC_INFO
		LOG(INFO, "initialization of liballoc " VERSION "\n" );
		#endif
		FLUSH();
		#endif
		...
		if ( l_memRoot == NULL )
		{
		  liballoc_unlock();
		  #ifdef LIBALLOC_DEBUG
		  LOG(ERR, "initial l_memRoot initialization failed\n", p); 
		  FLUSH();
		  #endif
		  return NULL;
		}
		...
		#ifdef LIBALLOC_INFO
		LOG(INFO, "set up first memory major %x\n", l_memRoot );
		FLUSH();
		#endif
	}
	...
	while ( maj != NULL )
	{
		...
		{
			#ifdef LIBALLOC_DEBUG
			LOG(NOHEADER, "CASE 1: Insufficient space in block %x\n", maj);
			FLUSH();
			#endif
		...
		if ( maj->first == NULL )
		{
			...
			#ifdef LIBALLOC_DEBUG
			LOG(NOHEADER, "CASE 2: returning %x\n", p); 
			FLUSH();
			#endif
		...
		if ( diff >= (size + sizeof(struct liballoc_minor)) )
		{
			...
			#ifdef LIBALLOC_DEBUG
			LOG(NOHEADER, "CASE 3: returning %x\n", p); 
			FLUSH();
			#endif
			liballoc_unlock();		// release the lock
			return p;
		...
		while ( min != NULL )
		{
				...
				if ( min->next == NULL )
				{
					...
					if ( diff >= (size + sizeof( struct liballoc_minor )) )
					{
						...
						#ifdef LIBALLOC_DEBUG
						LOG(NOHEADER, "CASE 4.1: returning %x\n", p); 
						FLUSH();
						#endif
						liballoc_unlock();		// release the lock
						return p;
				...
				if ( min->next != NULL )
				{
					...
					if ( diff >= (size + sizeof( struct liballoc_minor )) )
					{
						...
						#ifdef LIBALLOC_DEBUG
						LOG(NOHEADER, "CASE 4.2: returning %x\n", p); 
						FLUSH();
						#endif
		...
		{
			#ifdef LIBALLOC_DEBUG
			LOG(NOHEADER, "CASE 5: block full\n");
			FLUSH();
			#endif
	...
	#ifdef LIBALLOC_DEBUG
	LOG(ERR, "All cases exhausted. No memory available.\n");
	FLUSH();
	#endif
	#if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
	LOG(WARN, "WARNING: PREFIX(malloc)( %i ) returning NULL.\n", size);
	liballoc_dump();
	FLUSH();
	#endif
...
void PREFIX(free)(void *ptr)
{
	...
	if ( min->magic != LIBALLOC_MAGIC ) 
	{
		   ...
		   )
		{
			...
			#if defined LIBALLOC_DEBUG || defined LIBALLOC_INFO
			LOG(ERR, "ERROR: Possible 1-3 byte overrun for magic %x != %x\n",
								min->magic,
								LIBALLOC_MAGIC );
			FLUSH();
			#endif
