...
int ext2_mount(char *argument, char *mountpoint, struct fs_node** node_out) {
    ...
    // Allocate BGD memory
    ext2->bgds = kzalloc(sizeof(ext2_bgd_t) * ext2->bgd_count);

    // Calculate amount of BGD blocks
    size_t bgd_blocks = (sizeof(ext2_bgd_t) * ext2->bgd_count) / ext2->block_size;
    if (!bgd_blocks) bgd_blocks += 1;

    // Read the EXT2 block
    uint8_t *bgd_block;
    int r = ext2_readBlock(ext2, ext2->bgd_offset, &bgd_block);
    if (r) {
    ...
    for (unsigned i = 0; i < ext2->bgd_count; i++) {
        ...
        memcpy(EXT2_BGD(i), bgd, sizeof(ext2_bgd_t));
        bgd++;
    }
