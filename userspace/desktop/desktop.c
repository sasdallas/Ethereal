...
int main(int argc, char *argv[]) {
    ...
    int ch;
    int index;
    while ((ch = getopt_long(argc, argv, "bvh", (const struct option*)options, &index)) != -1) {
    ...
    // Create the background if we should
    if (!disable_bg) {
        create_background();
    }
    ...
    // Load the fonts
    taskbar_font = gfx_loadFont(taskbar_ctx, "/usr/share/DejaVuSansMono.ttf");

    // Fonts have been loaded, draw them in
    gfx_renderString(taskbar_ctx, taskbar_font, "Ethereal x86_64", 10, 15, GFX_RGB(255, 255, 255));
    ...
    while (1) {
        ...
        sleep(1); // will have to fix later
    }
